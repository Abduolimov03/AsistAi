...
print("[DEBUG] .env fayllari topildi. Bot ishga tayyor...")
...
MAX_MSG_LEN = 4000  # Telegram limitiga biroz kichikroq

async def send_long_text(message_obj, text, reply_markup=None):
    """Uzoq textni bo‘laklarga ajratib yuboradi"""
...
async def handle_message(message: types.Message):
    ...
    try:
        if reply_type == "text":
            ...
            ai_image_url = await get_ai_image(user_text)
            if ai_image_url:
                await typing.delete()
            ...
            ai_audio_file = await get_ai_audio(user_text)
            if ai_audio_file:
                await typing.delete()
...
if __name__ == "__main__":
    print("[DEBUG] Bot polling boshlanmoqda...")
